...
# ── Paths ──────────────────────────────────────────────────────
STUDIO_DIR    = Path(__file__).parent          # gs_param_studio/
SCRIPT_DIR    = STUDIO_DIR.parent              # project root
...
RENDER_SCRIPT = STUDIO_DIR / 'gs_ui_render.py'
...
@app.get('/')
async def index():
    return FileResponse(str(STUDIO_DIR / 'gs_ui.html'))
